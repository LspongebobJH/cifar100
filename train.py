...
if __name__ == '__main__':
    ...
    if args.ft_type == 'attn':
        ft = GAT(n_layers=1, n_heads_list=args.n_heads_list, in_dim=in_dim, 
                hid_dim=args.hid_dim, out_dim=num_classes, dropout=0.6, neg_slope=0.2).to(args.device)
    elif args.ft_type == 'ws':
        ft = WS(n_in_nodes=5, in_dim=in_dim, out_dim=num_classes).to(args.device)
    else: # args.ft_type == 'base'
        ft = nn.Linear(in_dim, num_classes).to(args.device)
